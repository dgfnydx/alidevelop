    ...
    antHelper.countDown({
        ...
        timeOutCallBack: function(totalTime, countDownTimer) {
            ...
            if(totalTime <= 0) {
                ...
                totalPoints({
                    total: total,
                    firstNode: "UI_04_defen004",
                    secondNode: "UI_04_defen003",
                    thirdNode: "UI_04_defen002",
                    forthNode: "UI_04_defen001",
                    pathPart1: "image/num1/UI_04_defen0",
                    pathPart2: "_Tex.png",
                    maxVal: 9
                })
...
function poseAnimate(time) {
    poseAnimateTimer = AR.setTimeout(function() {
        ...
        if(playingAmin[num] == 0) {
            ...
            if (Math.random() > 0.5) {
                moveX = Math.round(Math.random() * 15);
            }
            else {
                moveX = Math.round(Math.random() * -15);
            }
...
function getPose() {
    ...
    getPoseTimer = AR.setInterval(function() {
        for(var i = 1; i <= 12; i++) {
            var posePos = AR.get_position("attitude0" + i);
            // var posePosY = Math.floor(Math.abs(posePos.y));
            var posePosY = Math.floor(posePos.y)
                // AR.log("Y:" + posePosY);
            if(posePosY > -50 && posePosY < 30 && enGetNodeId) {
                // AR.log(node);
                // AR.modulate_alpha("attitude0" + i, 1, 0);
                startPoseLoop(i);
                enGetNodeId = false;
            } else if(posePosY > 31 && posePosY < 45) {
                // AR.modulate_alpha("attitude0" + i, 0.7, 0);
                AR.pose.stop();
...
function startPoseLoop(poseId) {
    ...
    AR.setTimeout(function() {
        ...
        if(maxScore >= 60 && maxScore < 80) {
            good++;
            per = 0;
            AR.set_visible("UI_03_GOOD", true);
            AR.set_visible("UI_03_DOOD_panding", true);
            AR.set_texture("", "image/num3/UI_03_panding0" + good + "_Tex.png", 0)
            AR.setTimeout(function() {
                AR.set_visible("UI_03_GOOD", false)
                AR.set_visible("UI_03_DOOD_panding", false)
            }, 700)
            AR.audio_play("audio/good.mp3");
        } else if(maxScore >= 80) {
            per++;
            good = 0;
            AR.set_visible("UI_03_PERFECT", true);
            AR.set_visible("UI_03_PERFECT_panding", true);
            AR.set_texture("", "image/num3num3/UI_03_panding0" + per + "_Tex.png", 0)
            AR.setTimeout(function() {
                AR.set_visible("UI_03_PERFECT", false)
                AR.set_visible("UI_03_PERFECT_panding", false)
            }, 700);
            AR.audio_play("audio/perfect.mp3");
        } else {
            good = 0;
            per = 0;
        }
        ...
        totalPoints({
            total: total,
            firstNode: "UI_03_fenshu04",
            secondNode: "UI_03_fenshu03",
            thirdNode: "UI_03_fenshu02",
            forthNode: "UI_03_fenshu01",
            pathPart1: "image/num2/UI_03_shuzi0",
            pathPart2: "_Tex.png",
            maxVal: 9
        })
...
function addScore(score) {
    if(score < 100) {
        var dig_1 = score % 10
        var dig_2 = Math.floor(score / 10) % 10;
        AR.set_texture("UI_03_jiafen03", "image/add/UI_03_jiafen" + dig_1 + "_Tex.png");
        AR.set_texture("UI_03_jiafen02", "image/add/UI_03_jiafen" + dig_2 + "_Tex.png");
    } else {
        AR.set_texture("UI_03_jiafen03", "image/add/UI_03_jiafen9_Tex.png");
        AR.set_texture("UI_03_jiafen02", "image/add/UI_03_jiafen9_Tex.png");
    }
    AR.setTimeout(function() {
        viscore(true)
        AR.setTimeout(function() {
    ...
    function viscore(bool) {
        for(var i = 1; i < 4; i++) {
            AR.set_visible("UI_03_jiafen0" + i, bool)
        }
    }
}

function totalPoints(tpConfig) {
    var totalPoint = tpConfig.total;
    if(totalPoint < 10000) {
        var dig_1 = totalPoint % 10;
        var dig_2 = Math.floor(totalPoint / 10) % 10;
        var dig_3 = Math.floor(totalPoint / 100) % 10;
        var dig_4 = Math.floor(totalPoint / 1000) % 10;
        AR.set_texture(tpConfig.firstNode, tpConfig.pathPart1 + dig_1 + tpConfig.pathPart2);
        AR.set_texture(tpConfig.secondNode, tpConfig.pathPart1 + dig_2 + tpConfig.pathPart2);
        AR.set_texture(tpConfig.thirdNode, tpConfig.pathPart1 + dig_3 + tpConfig.pathPart2);
        AR.set_texture(tpConfig.forthNode, tpConfig.pathPart1 + dig_4 + tpConfig.pathPart2);
    } else {
        AR.set_texture(tpConfig.firstNode, tpConfig.pathPart1 + tpConfig.maxVal + tpConfig.pathPart2);
        AR.set_texture(tpConfig.secondNode, tpConfig.pathPart1 + tpConfig.maxVal + tpConfig.pathPart2);
        AR.set_texture(tpConfig.thirdNode, tpConfig.pathPart1 + tpConfig.maxVal + tpConfig.pathPart2);
        AR.set_texture(tpConfig.forthNode, tpConfig.pathPart1 + tpConfig.maxVal + tpConfig.pathPart2);
    }
